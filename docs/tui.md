...
| **Video Rendering** | `timelapse render <target>` | **Render Tab** | **CLI**: Requires typing session paths or `"latest"`. Rendering output is printed to stdout/stderr.<br>**TUI**: Binds rendering to the active session selected (by pressing `[Space]` or `[Enter]` on it) in the Sessions Tab. Shows real-time rendering state (Ready, Rendering, Success, Error). FPS is adjustable via `[Up/Down]` arrow keys. Runs in a background thread to prevent TUI freeze. |
| **Sessions Listing** | `timelapse list` | **Sessions Tab** | **CLI**: Prints a static text list of all sessions in chronological order.<br>**TUI**: Displays an interactive table with columns (Name, Frames, Videos, Path). Supports arrow key navigation (`[Up/Down]`) to view metadata of different sessions, and pressing `[Space]` or `[Enter]` to set the active session (indicated with a `●`). Supports list refreshes (`[U]`). |
| **Open Session Folder** | `timelapse open <target>` | **Sessions Tab** (`[O]` key) | **CLI**: Opens specified folder directly.<br>**TUI**: Opens the highlighted folder under the cursor from the active row list directly in the system's default file manager. |
| **Session Cleanup** | `timelapse clean <target>` | **Sessions Tab** (`[C]` key) | **CLI**: Destructive commands require flags like `--frames` / `--videos` and manual CLI prompt confirmation.<br>**TUI**: Pressing `[C]` on the highlighted session displays a red popup modal overlay showing file counts. Pressing `[D]` runs a dry-run check, `[F]` cleans frames, `[V]` cleans videos, `[A]` cleans both, and any other key cancels. |
...
- **Welcome Screen**:
  - `[T]`: Toggle between **Extra** (rounded borders, layout emojis, gradient ASCII logo) and **Minimal** (simple centered text style) themes. Persists automatically to config.toml.
...
- **Tab-Specific Keys & Layout features**:
  ...
  - **Sessions Tab**:
    - `[Up/Down]`: Move cursor navigation pointer to highlight different sessions (shown with a `▶` prefix).
    - `[Space]` or `[Enter]`: Explicitly select the highlighted session as the active session for Capture/Render (marked with a `●` prefix).
    - `[O]`: Open the highlighted session folder.
    - `[C]`: Trigger clean popup modal for the highlighted session (where `[D]`, `[F]`, `[V]`, `[A]` select target to clean / dry-run, other keys cancel).
    - `[A]`: Set Capture mode to Append for this session, mark it as active, and jump to the Capture tab.
    - `[U]`: Reload sessions list from disk.
    - **Metadata Panel**: Shows the detailed metadata (Start time, Capture interval, Display, Backend, and active exclusions) for the currently highlighted session.
  - **Diagnostics Tab**:
